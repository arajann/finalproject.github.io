...
The final step in our model analysis was to stratify lung cancer incidence by gender to see if this improved the model fit. After running analysis for both models, we saw there were no significant improvements. Many of the coefficients in both models were insigificant, and there were no improvements in r^2 values. If we ran cross validations, I predict we would see similar high RMSE values like for the fitted model. 
